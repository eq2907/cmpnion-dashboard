  ...
  return (
    <div className="h-full flex flex-col fade-in">
      ...
      <div className='w-full h-full'>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mt-8 mb-8">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary w-full"
              />
            </div>
        ...
        <div className="max-h-127.5 lg:max-h-fit 2xl:max-h-245 flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                ...
                {orders.length === 0 ? (
                  ...
                  orders.map(order => {
                    ...
                    return (
                      ...
                      >
                        <td className="p-2 lg:p-4 font-medium text-slate-900">
                          {order.id}
                          {breached && <span className="lg:ml-2 inline-flex items-center text-xs font-bold text-red-600 animate-pulse"><AlertCircle size={12} className="mr-1" /> SLA</span>}
                        </td>
                        <td className="p-2 lg:p-4">{order.guestName}</td>
                        <td className="p-2 lg:p-4 font-bold">{order.roomNumber}</td>
                        <td className="p-2 lg:p-4">{order.service}</td>
                        <td className="p-2 lg:p-4 text-sm text-slate-500">
          ...
          <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium px-4 py-2 bg-white border border-slate-200 rounded-lg">
...
const OrderDetailsDrawer = ({ orderId, onClose, onAction, onCancel }: { orderId: string, onClose: () => void, onAction: (o: Order, s: OrderStatus) => void, onCancel: (id: string) => void }) => {
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-md bg-white h-full shadow-2xl p-6 flex flex-col ${isClosing ? 'slide-out-right' : 'slide-in-right'}`}
